...
<div>
    <ul class="uk-breadcrumb">
        ...
        <li><span>{{ucfirst($daerah)}}</span></li>
    </ul>
</div>
@if ($mean != 0)
<h4 class="uk-text-center">Harga rata rata di daerah {{ucfirst($daerah)}} Rp. {{number_format($mean)}}</h4>
@endif
<div class="uk-child-width-1-2@m uk-margin-bottom" uk-grid>
    <div class="uk-width-1-5@m">
        <form action="{{ Request::url() }}" method="get">
            ...
            @if(Request::url() != url('tanah/kota/lhokseumawe') && Request::url() != url('tanah/kota/aceh-utara'))
                <div class="uk-margin">
                    <select class="uk-select" name="kampung" id="">
                        <option value="">- Kampung -</option>
                        @foreach ($arrKampung as $kampung)
                            <option value="{{ $kampung->name }}" @if($kampung->name == $kampung) selected @endif>{{ $kampung->name }}</option>
                        @endforeach
                    </select>
                </div>
            @endif
